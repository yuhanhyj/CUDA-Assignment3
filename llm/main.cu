...
__device__ float exponentialIntegralFloatGPU(const int n, const float x, int maxIterations) {
...
__global__ void computeExponentialIntegral(float* result, int n, int numberOfSamples,
                                           float a, float b, int maxIterations) {
...
int main(int argc, char* argv[]) {
  cpu = true; gpu = true; verbose = false; timing = false;
  n = 10; numberOfSamples = 10;
  a = 0.0; b = 10.0;
  maxIterations = 50000;
  ...
  if (cpu) {
    ...
    if (timing) gettimeofday(&start, NULL);
    for (unsigned int i = 1; i <= n; ++i) {
      for (unsigned int j = 1; j <= numberOfSamples; ++j) {
        float x = a + j * ((b - a) / numberOfSamples);
        resultsCpuFloat[(i - 1) * numberOfSamples + (j - 1)] = exponentialIntegralFloatCPU(i, x);
      }
    }
    if (timing) {
      gettimeofday(&end, NULL);
      timeCpu = (end.tv_sec - start.tv_sec) + 1e-6 * (end.tv_usec - start.tv_usec);
    }
  }
  ...
  if (gpu) {
    ...
    struct timeval start, end;
    if (timing) gettimeofday(&start, NULL);
    computeExponentialIntegral<<<gridSize, blockSize>>>(d_results, n, numberOfSamples,
                                                        a, b, maxIterations);
    cudaMemcpy(resultsGpuFloat.data(), d_results, sizeof(float) * total, cudaMemcpyDeviceToHost);
    cudaFree(d_results);
    if (timing) {
      gettimeofday(&end, NULL);
      timeGpu = (end.tv_sec - start.tv_sec) + 1e-6 * (end.tv_usec - start.tv_usec);
    }
  }

  if (timing) {
    if (cpu) printf("CPU time: %f seconds\n", timeCpu);
    if (gpu) printf("GPU time: %f seconds\n", timeGpu);
  }
  ...
  if (verbose) {
    for (unsigned int i = 1; i <= n; ++i) {
      for (unsigned int j = 1; j <= numberOfSamples; ++j) {
        ...
        printf("n=%u, x=%.6f, CPU=%.6f, GPU=%.6f, Δ=%.6e\n", i, x, cpuVal, gpuVal, fabs(cpuVal - gpuVal));
      }
    }
  }
